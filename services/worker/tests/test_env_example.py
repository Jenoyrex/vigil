"""Guards services/worker/.env.example against silently drifting from
worker.config.Settings -- Phase 4A. Mechanical and narrow by design: this
only asserts every current Settings field has a corresponding `KEY=` line
in the example file, so adding a new setting without updating the template
fails this test instead of being discovered by a confused new contributor.
"""
...
def test_every_settings_field_has_an_env_example_entry() -> None:
    ...
    assert not missing, f"services/worker/.env.example is missing: {sorted(missing)}"
